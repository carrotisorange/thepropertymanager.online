  ...
  <div class="col-md-12">
    <nav>
      <div class="nav nav-tabs" id="nav-tab" role="tablist">
        <a class="nav-item nav-link active" id="nav-unit-tab" data-toggle="tab" href="#unit" role="tab"
          aria-controls="nav-unit" aria-selected="true"><i class="fas fa-home text-indigo"></i> Unit</a>
        <a class="nav-item nav-link" id="nav-tenant-tab" data-toggle="tab" href="#occupants" role="tab"
          aria-controls="nav-occupants" aria-selected="false"><i class="fas fa-users text-green"></i> Occupants</a>
        <a class="nav-item nav-link" id="nav-owners-tab" data-toggle="tab" href="#owners" role="tab"
          aria-controls="nav-owners" aria-selected="false"><i class="fas fa-user-tie text-teal"></i> Owners</a>
        @if($bills->count() <= 0) <a class="nav-item nav-link" id="nav-bills-tab" data-toggle="tab" href="#bills"
          ...
          <a class="nav-item nav-link" id="nav-payments-tab" data-toggle="tab" href="#payments" role="tab"
            aria-controls="nav-payments" aria-selected="false"><i class="fas fa-coins text-yellow"></i> Payments </a>

          <a class="nav-item nav-link" id="nav-concerns-tab" data-toggle="tab" href="#concerns" role="tab"
            aria-controls="nav-concerns" aria-selected="false"><i class="fas fa-tools fa-sm text-cyan"></i> Concerns
          </a>
      </div>
    </nav>
  </div>
  ...
  <div class="col-md-12">
    <div class="tab-content" id="nav-tabContent">
      ...
      <div class="tab-pane fade show active" id="unit" role="tabpanel" aria-labelledby="nav-unit-tab">
        ...
        <button type="button" title="edit unit" class="btn btn-primary btn-sm" data-toggle="modal"
          data-target="#editUnit" data-whatever="@mdo"><i class="fas fa-edit"></i> Edit</button>
              ...
              <tr>
                ...
                <td>
                  @if($home->floor <= 0) {{ $numberFormatter->format($home->floor * -1) }} basement @else
                    {{ $numberFormatter->format($home->floor) }} floor @endif </td> </tr> <tr>
                <th>Type</th>
                <td>{{ $home->type }}</td>
              </tr>
      ...
      <div class="tab-pane fade" id="bills" role="tabpanel" aria-labelledby="nav-bills-tab">
        <a href="#" data-toggle="modal" data-target="#addBill" class="btn btn-primary btn-sm"><i
            class="fas fa-plus"></i> New</a>
        @if(Auth::user()->role_id_foreign === 3 || Auth::user()->role_id_foreign === 4 || Auth::user()->role_id_foreign
        === 1)
        <a href="/property/{{Session::get('property_id')}}/unit/{{ $home->unit_id }}/bills/edit"
          class="btn btn-primary btn-sm"><i class="fas fa-edit"></i> Edit</a>
        @endif
        @if($bills->count() > 0)
        <a target="_blank" href="/property/{{Session::get('property_id')}}/unit/{{ $home->unit_id }}/bills/export"
          class="btn btn-primary btn-sm"><i class="fas fa-download"></i> Export</span></a>
            ...
            <table class="table table-hover table-bordered">
              ...
              <tr>
                ...
                {{-- <td class="text-center">
                  ...
                  <form action="/property/{{Session::get('property_id')}}/tenant/{{ $item->bill_tenant_id }}/bill/{{ $item->billing_id }}"
                method="POST">
                @csrf
                @method('delete')
      ...
      <div class="tab-pane fade" id="payments" role="tabpanel" aria-labelledby="nav-payments-tab">
        ...
        === 1)
        <a href="#" data-toggle="modal" data-target="#acceptPayment" class="btn btn-primary btn-sm"><i
            class="fas fa-plus"></i> New</a>
        @endif
        <br><br>
        <div class="col-md-12 mx-auto">
          <div class="table-responsive text-nowrap" style="overflow-y:scroll;overflow-x:scroll;">
            <table class="table table-hover">
              ...
              <thead>
                {{-- <tr>
                      <th colspan="10">{{ Carbon\Carbon::parse($day)->addDay()->format('M d Y') }}
                ({{ $collection_list->count() }})</th>
              ...
              <tr>
                ...
                {{-- <td class="text-center">
                          @if(Auth::user()->role_id_foreign === 5 || Auth::user()->role_id_foreign === 4)
                           <form action="/property/{{$property->property_id}}/home/{{ $item->unit_id }}/payment/{{ $item->payment_id }}"
                method="POST">
                @csrf
                @method('delete')
                ...
                <td class="text-center">
                  <a title="export" target="_blank"
                    href="/property/{{Session::get('property_id')}}/home/{{ $item->bill_unit_id }}/payment/{{ $item->payment_id }}/dates/{{$item->payment_created}}/export"
                    class="btn btn-sm btn-primary"><i class="fas fa-download fa-sm text-white-50"></i></a>


                </td> --}}
                <td>
                  <a title="export" target="_blank"
                    href="/property/{{ Session::get('property_id') }}/unit/{{ $item->unit_id }}/tenant/{{ $item->tenant_id }}/payment/{{ $item->payment_id }}/dates/{{$item->payment_created}}/export_unit_bills"
                    class="btn btn-sm btn-primary"><i class="fas fa-download fa-sm text-white-50"></i></a>
                </td>
      ...
      <div class="tab-pane fade" id="occupants" role="tabpanel" aria-labelledby="nav-occupants-tab">
        ...
        @if($owners->count() <= 0) <a href="#" data-toggle="modal" data-target="#modalToAddOwner"
          class="btn btn-primary btn-sm"> <i class="fas fa-user-plus"></i> New </a>
          @else
          <a href="#" data-toggle="modal" data-target="#addOccupant" class="btn btn-primary btn-sm"> <i
              class="fas fa-user-plus"></i> New</a>
          @endif
          @else
          <a href="/property/{{Session::get('property_id')}}/unit/{{ $home->unit_id }}/occupant/add" type="button"
            class="btn btn-primary btn-sm"><i class="fas fa-user-plus"></i> New</a>
          @endif
              ...
              <table class="table table-table-bordered table-hover">
                ...
                <tbody>
                  ...
                  <tr>
                    <th>{{ $occupanct_ctr++ }}</th>
                    <th><a
                        href="/property/{{ Session::get('property_id') }}/occupant/{{ $item->tenant_id }}/">{{ $item->first_name.' '.$item->middle_name.' '.$item->last_name }}</a>
                    </th>
      ...
      <div class="tab-pane fade" id="owners" role="tabpanel" aria-labelledby="nav-owners-tab">
        ...
        <a data-toggle="modal" data-target="#addInvestor" data-whatever="@mdo" type="button"
        ...
        <div class="col-md-12 mx-auto">
          ...
          <div class="table-responsive text-nowrap" style="overflow-y:scroll;overflow-x:scroll;">
            <table class="table table-table-bordered table-hover">
              ...
              <tr>
                <th>{{ $ctr++ }}</th>
                <td><a href="/property/{{Session::get('property_id')}}/owner/{{ $item->owner_id }}">{{ $item->name }}
                  </a></td>
...
<div class="modal fade" id="editUnit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true"
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form id="editUnitForm" action="/property/{{ Session::get('property_id') }}/unit/{{ $home->unit_id }}"
        method="POST">
        @method('put')
        @csrf
      </form>
        ...
        <form>
          ...
          <div class="form-group">
            ...
            <input oninput="this.value = Math.abs(this.value)" form="editUnitForm" type="number"
              value="{{ $home->occupancy }}" name="occupancy" class="form-control">
          </div>
          <div class="form-group">
            <label> Status</label>
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" form="concernForm" class="btn btn-primary"
          onclick="return confirm('Are you sure you want perform this action?'); this.disabled = true;"> Add</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="addOccupant" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  aria-hidden="true" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-md" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-footer">
        <a href="/property/{{Session::get('property_id')}}/unit/{{ $home->unit_id }}/occupant" type="button"
          class="btn btn-secondary btn-sm"><i class="fas fa-times"></i> No</a>
        <a href="/property/{{Session::get('property_id')}}/unit/{{ $home->unit_id }}/occupant/prefilled" type="button"
          class="btn btn-primary btn-sm"><i class="fas fa-check"></i> Yes</a>
      </div>
...
<div class="modal fade" id="modalToAddOwner" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  aria-hidden="true" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-md" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-footer">
        ...
        <a data-toggle="modal" data-target="#addInvestor" id="addOwnerForm" data-whatever="@mdo" type="button"
          class="btn btn-primary text-white">Add Now</a>
      </div>
...
<div class="modal fade" id="addBill" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true"
  ...
  <div class="modal-dialog modal-xl" role="modal">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="addBillForm" action="/property/{{Session::get('property_id')}}/unit/{{ $home->unit_id }}/bills/create"
          method="POST">
          @csrf
        </form>
        ...
        <div class="row">
          <div class="col-md-3">
            <label>Date</label>
            {{-- <input type="date" form="addBillForm" class="form-control" name="date_posted" value="{{ Carbon\Carbon::now()->format('Y-m-d') }}"
            required > --}}
            <input class="form-control" type="date" form="addBillForm" class="" name="date_posted"
              value="{{ Carbon\Carbon::now()->format('Y-m-d') }}" required>
          </div>
          <div class="col">
            <p class="text-right">
      ...
      <div class="modal-footer">
        <button form="addBillForm" type="submit" class="btn btn-primary btn-sm"
...
<div class="modal fade" id="acceptPayment" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  aria-hidden="true" data-backdrop="static" data-keyboard="false">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="acceptPaymentForm"
          action="/property/{{Session::get('property_id')}}/home/{{ $home->unit_id }}/collection" method="POST">
          @csrf
        </form>
        ...
        <div class="row">
          <div class="col-md-3">
            ...
            {{-- <input form="acceptPaymentForm" type="date" class="form-control" name="payment_created" value={{date('Y-m-d')}}
            required> --}}
            <input type="date" form="acceptPaymentForm" class="form-control" name="payment_created"
              value="{{ Carbon\Carbon::now()->format('Y-m-d') }}" required>
          </div>
          <div class="col">
            <p class="text-right">
      ...
      <div class="modal-footer">
        <button form="acceptPaymentForm" id="addPaymentButton" type="submit" class="btn btn-primary btn-sm"
          onclick="return confirm('Are you sure you want perform this action?'); this.disabled = true;"><i
            class="fas fa-check"></i> Submit</button>
      </div>
