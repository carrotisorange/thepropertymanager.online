    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a
            href="/property/{{ Session::get('property_id') }}/tenant/{{ $contract->tenant_id_foreign }}/#contracts"">{{ $tenant->first_name.' '.$tenant->last_name }}</a></li>
        <li class=" breadcrumb-item"><a
...
<div class="row">
  <form id="editContractForm"
    action="/property/{{Session::get('property_id')}}/tenant/{{ $contract->tenant_id_foreign }}/contract/{{ $contract->contract_id }}/update"
    method="POST">
    @csrf
    @method('PUT')
  </form>
  <div class="col-md-12">
    ...
    <div class="form-group row">
      <div class="col">
        ...
        <select form="editContractForm" class="form-control" name="referrer_id" id="referrer_id">
          <option value="{{ $contract->referrer_id? $contract->referrer_id: '36' }}" selected>
            {{ $contract->referrer_id }}</option>
    ...
    <div class="form-group row">
      <div class="col">
        <label>Rent</label>
        <input form="editContractForm" type="number" step="0.001" name="rent" id="rent" class="form-control"
          value="{{ $contract->rent }}">
      </div>
    ...
    <div class="form-group row">
      <div class="col">
        <label>Movein</label>
        <input form="editContractForm" type="date" name="movein_at" id="movein_at" onkeyup='autoFill()'
          class="form-control" value="{{ Carbon\Carbon::parse($contract->movein_at)->format('Y-m-d') }}">
      </div>
      <div class="col">
        <label>Moveout</label>
        <input form="editContractForm" type="date" name="moveout_at" id="moveout_at" onkeyup='autoFill()'
          class="form-control" value="{{ Carbon\Carbon::parse($contract->moveout_at)->format('Y-m-d') }}">
        <input form="editContractForm" type="hidden" class="form-control" name="number_of_months" id="number_of_months"
          required readonly value="{{ $contract->number_of_months? $contract->number_of_months: 'NULL' }}">
        <input form="editContractForm" type="hidden" class="form-control" name="term" id="term"
          value="{{ $contract->term? $contract->term: 'NULL' }}" required readonly>
      </div>
      <div class="col">
        <label for="">Actual Moveout Date</label>
        <input form="editContractForm" type="date" class="form-control" name="actual_moveout_at" id="actual_moveout_at"
          value="{{ $contract->actual_moveout_at? Carbon\Carbon::parse($contract->moveout_at)->format('Y-m-d'): Carbon\Carbon::parse($contract->moveout_at)->format('Y-m-d') }}">
      </div>
...
<div class="row">
  <div class="col">
    <p class="text-right">
      ...
      <button form="editContractForm" type="submit" class="btn btn-primary btn-sm"
