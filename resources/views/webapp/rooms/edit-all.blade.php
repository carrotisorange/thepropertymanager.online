    ...
    <div class="row" style="overflow-y:scroll;overflow-x:scroll;height:450px;">
      ...
      <table class="table table-condensed table-bordered table-hover">
        ...
        <tbody>
          ...
          <tr>
            <th> {{ $ctr++ }}</th>
            <td><input form="editUnitsForm" type="text" name="building{{ $building++  }}" id=""
                value="{{ $item->building }}"></td>
            <td>
              <input form="editUnitsForm" type="text" name="unit_no{{ $unit_no++  }}" id=""
                value="{{ $item->unit_no }}">
            ...
            <td><input form="editUnitsForm" type="number" name="occupancy{{ $occupancy++  }}" id="" min="0"
                value="{{ $item->occupancy }}"></td>
            <td><input form="editUnitsForm" type="number" step="0.001" name="rent{{ $rent++  }}" min="0" id=""
                value="{{$item->rent }}"></td>
            <td>
            ...
            <td>
              ...
              <form action="/property/{{Session::get('property_id')}}/room/{{ $item->unit_id }}/restore" method="POST">
                ...
                <button title="restore this room" type="submit"
                  class="d-none d-sm-inline-block btn btn-sm btn-success shadow-sm"
                  onclick="return confirm('Are you sure you want perform this action?');"><i
                    class="fas fa-trash-restore fa-sm text-dark-50"></i></button>
              </form>
              @else
              <form action="/property/{{Session::get('property_id')}}/room/{{ $item->unit_id }}/delete" method="POST">
                ...
                <button title="archive this room" type="submit"
                  class="d-none d-sm-inline-block btn btn-sm btn-danger shadow-sm"
                  onclick="return confirm('Are you sure you want perform this action?');"><i
                    class="fas fa-trash fa-sm text-dark-50"></i></button>
              </form>
              @endif
            </td>
          </tr>
