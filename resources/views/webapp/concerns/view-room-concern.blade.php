...
<form id="updateConcernForm"
  action="/property/{{ Session::get('property_id') }}/room/{{ $room->unit_id }}/concern/{{ $concern->concern_id }}/update"
  method="POST">
  @csrf
  @method('PUT')
</form>
  ...
  <div class="col-md-10 py-3 mx-auto">
    <div class="card">
      <div class="card-body">
        <div class="form-row">
          <div class="form-group col-md-3">
            ...
            <input form="updateConcernForm" name="reported_at" class="form-control" type="date"
              value="{{ old('reported_at')? old('reported_at'): Carbon\Carbon::parse($concern->reported_at)->format('Y-m-d') }}"
              required>
            @error('reported_at')
          ...
          <div class="form-group col-md-3">
            ...
            <input form="updateConcernForm" value="{{ $room->building.' '.$room->unit_no }}" class="form-control"
              type="text" required readonly>
            <input form="updateConcernForm" name="concern_unit_id" value="{{ $room->unit_id }}" class="form-control"
          ...
          <div class="form-group col-md-3">
            ...
            <select form="updateConcernForm" class="form-control" name="concern_tenant_id">
              <option value="{{ old('concern_tenant_id')? old('concern_tenant_id'): $tenant->tenant_id }}" selected>
          ...
          <div class="form-group col-md-3">
            ...
            <input name="contact_no" form="updateConcernForm"
              value="{{ old('contact_no')? old('contact_no'): $concern->contact_no }}" class="form-control"
          ...
          <div class="form-row">
            <div class="form-group col-md-12">
              ...
              <textarea form="updateConcernForm" name="details" cols="115%" rows="5"
                class="form-control">{{ old('details')? old('details'): $concern->details }}</textarea>
              @error('details')
              <small class="text-danger">
                {{ $message }}
              </small>
        ...
        <div class="form-row">
          <div class="form-group col-md-3">
            ...
            <select form="updateConcernForm" name="urgency" class="form-control" required>
              <option value="{{ old('urgency')? old('urgency'): $concern->urgency }}" selected>
                {{ old('urgency')? old('urgency'): $concern->urgency }}</option>
          ...
          <div class="form-group col-md-3">
            ...
            <input name="scheduled_at" form="updateConcernForm"
              value="{{ old('scheduled_at')? old('scheduled_at'): Carbon\Carbon::parse($concern->scheduled_at)->format('Y-m-d') }}"
          ...
          <div class="form-group col-md-3">
            ...
            <select form="updateConcernForm" class="form-control" name="concern_user_id">
              <option value="{{ old('concern_user_id')? old('concern_user_id'): $endorsed_to->id }}">
                {{ old('concern_user_id')? old('concern_user_id'): $endorsed_to->name }}</option>
        ...
        <div class="form-row">
          <div class="form-group col-md-12">
            ...
            <textarea form="updateConcernForm" name="action_taken" cols="115%" rows="5"
              class="form-control">{{ old('concern_user_id')? old('action_taken'): $concern->action_taken }}</textarea>
            @error('action_taken')
            <small class="text-danger">
              {{ $message }}
            </small>
        ...
        <div class="form-row">
          ...
          <div class="form-group col-md-3">
            ...
            <select form="updateConcernForm" class="form-control" name="resolved_by">
              <option value="{{ old('resolved_by')? old('resolved_by'): $resolved_by->id }}">
                {{ old('resolved_by')? old('resolved_by'): $resolved_by->name }}</option>
              @foreach ($users as $item)
              <option value="{{ $item->id }}">{{ $item->name }}</option>
              @endforeach
            </select>
          ...
          <div class="form-group col-md-3">
            ...
            <select form="updateConcernForm" id="status" name="status" class="form-control" required>
              <option value="{{ old('status')? old('status'): $concern->status }}" selected>
                {{ old('status')? old('status'): $concern->status }}</option>
          ...
          <div class="form-group col-md-3">
            ...
            <input name="resolved_at" form="updateConcernForm"
              value="{{ old('resolved_at')? old('resolved_at'): Carbon\Carbon::parse($concern->resolved_at)->format('Y-m-d') }}"
              class="form-control" type="date" required>
            @error('resolved_at')
            <small class="text-danger">
              {{ $message }}
          ...
          <div class="form-group col-md-3">
            ...
            <select form="updateConcernForm" id="rating" name="rating" class="form-control" required>
              <option value="{{ old('rating')? old('rating'): $concern->rating }}" selected>
                {{ old('rating')? old('rating'): $concern->rating }}</option>
        ...
        <div class="form-row">
          <div class="form-group col-md-12">
            ...
            <textarea form="updateConcernForm" name="remarks" cols="115%" rows="5"
              class="form-control">{{ old('remarks')? old('remarks'): $concern->remarks }}</textarea>
            @error('remarks')
            <small class="text-danger">
              {{ $message }}
            </small>
      ...
      <div class="form-group col-md-11 mx-auto">
        <button type="submit" form="updateConcernForm" class="btn btn-primary btn-block"
    ...
    <p class="text-center"> <a href="#" data-toggle="modal" data-target="#addResponse"><i class="fas fa-plus"></i>
        Response</a> </p>
