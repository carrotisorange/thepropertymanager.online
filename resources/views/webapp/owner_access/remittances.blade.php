...
<div class="table-responsive text-nowrap">
  @if($remittances->count() <=0 ) <p class="text-danger text-center">No remittances found!</p>
    @else
    <table class="table">
      <thead>
        <?php $ctr=1;?>
      ...
      <tbody>
        ...
        <tr>
          ...
          <th><a
              href="/user/{{ Auth::user()->id }}/owner/{{ $owner->owner_id }}/remittance/{{ $item->remittance_id }}/expenses">{{ number_format($item->amt_remitted,2) }}</a>
          </th>
        </tr>
        @endforeach
