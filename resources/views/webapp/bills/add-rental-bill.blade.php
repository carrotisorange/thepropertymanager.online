  ...
  <table class="table table-condensed table-bordered table-hover">
   ...
   ?>
    ...
    <tr>
      ...
      <input class="" type="hidden" form="add_billings" name="particular{{ $desc_ctr++ }}" value="Rent" required
        readonly>
      <td>
        <?php 
        ...
        <input form="add_billings" type="number" name="amount{{ $amt_ctr++ }}" step="0.01"
          value="{{ $prorated_monthly_rent }}" oninput="this.value = Math.abs(this.value)" required>
        @else
        <input form="add_billings" type="number" name="amount{{ $amt_ctr++ }}" step="0.01"
          value="{{ $item->contract_rent }}" oninput="this.value = Math.abs(this.value)" required>
        @endif
      </td>
      {{-- <td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="periodCoveredForm"
          action="/property/{{Session::get('property_id')}}/bills/rent/{{ $updated_start? Carbon\Carbon::parse($updated_start)->format('Y-m-d'): null }}-{{ Carbon\Carbon::parse($updated_end)->format('Y-m-d') }}/"
          method="POST">
          @csrf
          <div class="row">
            <div class="col">
