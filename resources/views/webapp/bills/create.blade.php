      ...
      <div class="card-body">
        <div class="form-row">
          <div class="form-group col-md-4">
            ...
            <input form="createBillForm" value="{{ $room->building.' '.$room->unit_no }}" class="form-control"
              type="text" required readonly>

          </div>
          <div class="form-group col-md-4">
            <label for="">Beds:</label>
            <input form="createBillForm" value="{{ $room->occupancy }} pax" class="form-control" type="text" required
              readonly>
          </div>
          <div class="form-group col-md-4">
            <label for="">Rent/mo:</label>
            <input form="createBillForm" value="{{ $contract->rent }}" class="form-control" type="number" required
              readonly>
          </div>
        </div>
        <div class="form-row">
          <div class="form-group col-md-12">
            ...
            <input form="createBillForm"
              value="{{ $tenant->first_name.' '.$tenant->middle_name.' '.$tenant->last_name }}" class="form-control"
              type="text" required readonly>
        ...
        <div class="form-row">
          <div class="form-group col-md-3">
            ...
            <input name="movein_at" id="movein_at" form="createBillForm"
              value="{{ Carbon\Carbon::parse($contract->movein_at)->format('M d, Y') }}" class="form-control"
              type="text" required readonly>

          </div>
          <div class="form-group col-md-3">
            <label for="">Move out:</label>
            <input name="moveout_at" id="moveout_at" form="createBillForm"
              value="{{ Carbon\Carbon::parse($contract->moveout_at)->format('M d, Y') }}" class="form-control"
              type="text" required readonly>

          </div>
          <div class="form-group col-md-3">
            <label for="">Lenght of stay:</label>
            <input name="number_of_months" id="number_of_months" form="createBillForm"
              value="{{ $contract->number_of_months }}" class="form-control" type="text" required readonly>
          ...
          <div class="form-group col-md-3">
            ...
            <input name="term" id="term" form="createBillForm" value="{{ $contract->term }}" class="form-control"
              type="text" required readonly>
        ...
        <form id="createBillForm"
          action="/property/{{ Session::get('property_id') }}/room/{{ $room->unit_id }}/tenant/{{ $tenant->tenant_id }}/store/bill"
          method="POST">
          @csrf
        </form>
            ...
            <tr>
              ...
              <td>
                <select class="form-control" form="createBillForm" name="particular_id_foreign" id="">
                  <option value="{{ old('particular_id_foreign') }}" selected>{{ old('particular_id_foreign') }}
                  </option>
              ...
              <td>
                <input form="createBillForm" type="date" value="{{ old('start') }}" name="start" id="start"
                  class="form-control">
                @error('start')
                <small class="text-danger">
                  {{ $message }}
              ...
              <td>
                <input form="createBillForm" type="number" min="1" value="{{ old('amount') }}" step="0.001"
              ...
              <td> <button form="createBillForm" type="submit" class="btn btn-primary"
                  onclick="this.form.submit(); this.disabled = true;"><i class="fas fa-check"></i> Add</button></td>
            </tr>
          </table>
        </div>
