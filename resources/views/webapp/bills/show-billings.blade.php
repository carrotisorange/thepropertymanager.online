...
<nav>
  <div class="nav nav-tabs" id="nav-tab" role="tablist">
    <a class="nav-item nav-link active" id="nav-bills-tab" data-toggle="tab" href="#bills" role="tab"
      aria-controls="nav-bills" aria-selected="true"><i class="fas fa-file-invoice-dollar fa-sm text-primary-50"></i>
      Bills <span class="badge badge-primary badge-counter">{{ $balance->count() }}</span></a>
    <a class="nav-item nav-link" id="nav-payments-tab" data-toggle="tab" href="#payments" role="tab"
      aria-controls="nav-payments" aria-selected="false"> <i class="fas fa-money-bill fa-sm text-primary-50"></i>
      Payments </a>
...
<div class="tab-content" id="nav-tabContent">
  ...
  <div class="tab-pane fade show active" id="bills" role="tabpanel" aria-labelledby="nav-bills-tab">
    ...
    @else
    <a href="/units/{{ $tenant->unit_tenant_id }}/tenants/{{ $tenant->tenant_id }}" class="btn btn-primary"><i
        class="fas fa-arrow-left fa-sm text-white-50"></i> Tenant</a>
    @endif

    @if(Auth::user()->role_id_foreign === 3 || Auth::user()->role_id_foreign === 4 || Auth::user()->role_id_foreign ===
    1)
    <a href="/units/{{ $tenant->unit_tenant_id }}/tenants/{{ $tenant->tenant_id }}/billings/edit"
      class="btn btn-primary"><i class="fas fa-edit fa-sm text-gray-50"></i> Edit</a>
    @endif

    @if(Auth::user()->role_id_foreign === 5 || Auth::user()->role_id_foreign === 4 || Auth::user()->role_id_foreign ===
    3 || Auth::user()->role_id_foreign === 1)

    @if($balance->count() > 0)
    {{-- <a  target="_blank" href="/units/{{ $tenant->unit_tenant_id }}/tenants/{{ $tenant->tenant_id }}/bills/download"
    class="btn btn-primary"><i class="fas fa-download fa-sm text-white-50"></i> Export </span></a>
    @if($tenant->email_address !== null)
    <a target="_blank" href="/units/{{ $tenant->unit_tenant_id }}/tenants/{{ $tenant->tenant_id }}/bills/send"
      class="btn btn-primary"><i class="fas fa-paper-plane  fa-sm text-white-50"></i> Send </span></a>
    @endif --}}
    @endif
    ...
    <div class="row">
      <div class="col-md-11 mx-auto">
        ...
        <p>
          ...
          <br>
          <span class="text-danger"><b>Due Date:</b>
          ...
          <table class="table">
            ...
            <tr>
              ...
              <th class="text-right text-danger">
                {{ number_format($balance->sum('balance') + ($balance->sum('balance') * .1) ,2) }}</th>
            </tr>
            @endif
          </table>
        </div>
  ...
  <div class="tab-pane fade" id="payments" role="tabpanel" aria-labelledby="nav-payments-tab">
    ...
    @if(Auth::user()->role_id_foreign === 5 || Auth::user()->role_id_foreign === 4 || Auth::user()->role_id_foreign ===
    1)
    <a href="#" data-toggle="modal" data-target="#acceptPayment" class="btn btn-primary"><i
      ...
      <div class="col-md-11 mx-auto">
        ...
        <div class="table-responsive text-nowrap">
          <table class="table">
            ...
            <tr>
              <th colspan="12">{{ Carbon\Carbon::parse($day)->addDay()->format('M d Y') }}
            ...
            <tr>
              ...
              <td class="text-center">
                <a title="export" target="_blank"
                  href="/units/{{ $item->unit_tenant_id }}/tenants/{{ $item->tenant_id }}/payments/{{ $item->payment_id }}/dates/{{$item->payment_created}}/export"
              ...
              <td>
                ...
                <form action="/tenants/{{ $item->tenant_id }}/payments/{{ $item->payment_id }}" method="POST">
                  @csrf
                  @method('delete')
                  <button title="remove" type="submit" class="d-none d-sm-inline-block btn btn-sm btn-danger shadow-sm"
...
<div class="modal fade" id="editPaymentFooter" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button form="editPaymentFooterForm" type="submit" class="btn btn-primary"
...
<div class="modal fade" id="acceptPayment" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  ...
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row">
          <div class="col-md-6">
            ...
            required> --}}
            <input class='form-control col-md-6' type="date" form="acceptPaymentForm" class="" name="payment_created"
              value="{{ Carbon\Carbon::now()->format('Y-m-d') }}" required>
          </div>
          <div class="col-md-6">
            <small for="">Acknowledgment Receipt No</small>
            <input class='form-control col-md-6' form="acceptPaymentForm" type="text" class="" id="" name="ar_no"
              value="{{ $payment_ctr }}" required readonly>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-md-12">
            ...
            <p class="text-left">
              <a href="#/" id='delete_payment' class="d-none d-sm-inline-block btn btn-sm btn-danger shadow-sm"><i
                  class="fas fa-times-circle fa-sm text-white-50"></i> Remove</a>
              <a href="#/" id="add_payment" class="btn btn-primary"><i
                  class="fas fa-check-circle fa-sm text-white-50"></i> Add</a>
            </p>
            <div class="table-responsive text-nowrap">
        ...
        <input type="hidden" form="acceptPaymentForm" id="payment_tenant_id" name="payment_tenant_id"
          value="{{ $tenant->tenant_id }}">
        <input type="hidden" form="acceptPaymentForm" id="unit_tenant_id" name="unit_tenant_id"
          value="{{ $tenant->unit_tenant_id }}">
        <input type="hidden" form="acceptPaymentForm" id="tenant_status" name="tenant_status"
      ...
      <div class="modal-footer">
        {{-- <button type="button" class="d-none d-sm-inline-block btn btn-sm btn-secondary shadow-sm" data-dismiss="modal"><i class="fas fa-times fa-sm text-white-50"></i> Cancel</button> --}}
        <button form="acceptPaymentForm" id="addPaymentButton" type="submit" class="btn btn-primary"
          onclick="return confirm('Are you sure you want perform this action?'); this.disabled = true;"><i
            class="fas fa-check fa-sm text-white-50f"></i> Submit</button>
      </div>
  ...
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row">
          <div class="col">
            ...
            required > --}}
            <input type="date" form="addBillForm" class="" name="date_posted"
              value="{{ Carbon\Carbon::parse($tenant->movein_date)->format('Y-m-d') }}" required>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col">
            ...
            <p class="text-left">
              <span id='delete_row' class="d-none d-sm-inline-block btn btn-sm btn-danger shadow-sm"><i
                  class="fas fa-minus fa-sm text-white-50"></i> Remove Bill</span>
      ...
      <div class="modal-footer">
        ...
        <button form="addBillForm" type="submit" class="btn btn-primary"
          onclick="return confirm('Are you sure you want perform this action?'); this.disabled = true;"><i
            class="fas fa-check fa-sm text-white-50"></i> Submit</button>
      </div>
    </div>
