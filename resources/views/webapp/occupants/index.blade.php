...
@if($tenants->count() <=0 ) <p class="text-danger text-center">No occupants found!</p>

  @else
  Showing <b>{{ $tenants->count() }} </b> of {{ $count_tenants }} occupants
  @if(Session::get('tenant_search'))
  <p class="text-center"> <span class=""> <small> you searched for </small></span> <span
      class="text-danger">{{ Session::get('tenant_search') }}"<span></p>
  @endif
