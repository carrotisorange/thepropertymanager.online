        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a
                        href="/property/{{ Session::get('property_id') }}/occupant/{{ $tenant->tenant_id }}">{{ $tenant->first_name.' '.$tenant->last_name }}</a>
                </li>
...
<form id="editTenantForm" action="/property/{{Session::get('property_id')}}/occupant/{{ $tenant->tenant_id }}"
    method="POST">
    @method('put')
    @csrf
</form>
...
<p class="text-right">
    {{-- <a href="/property/{{Session::get('property_id')}}/tenant/{{ $tenant->tenant_id }}" class="btn
    btn-secondary"><i class="fas fa-times fa-sm text-dark-50"></i> Cancel</a> --}}
    <button type="submit" form="editTenantForm" class="btn btn-primary btn-sm"
        onclick="return confirm('Are you sure you want perform this action?'); this.disabled = true;"><i
            class="fas fa-check fa-sm text-white-50"></i> Update</button>
</p>
@endsection
